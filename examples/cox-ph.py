...
durations_sub_test = durations_test[76:]
durations_test = durations_test[:76]
...
for row in range(x_sub_test.shape[0]):
    ...
    sort_prediction = np.copy(prediction)
    prediction.reshape(-1).sort()
    ...
    durations_test = np.asarray(durations_test_list, dtype=np.float32)
